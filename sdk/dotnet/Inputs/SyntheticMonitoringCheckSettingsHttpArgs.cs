// *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.Grafana.Inputs
{

    public sealed class SyntheticMonitoringCheckSettingsHttpArgs : global::Pulumi.ResourceArgs
    {
        [Input("basicAuth")]
        public Input<Inputs.SyntheticMonitoringCheckSettingsHttpBasicAuthArgs>? BasicAuth { get; set; }

        [Input("bearerToken")]
        public Input<string>? BearerToken { get; set; }

        [Input("body")]
        public Input<string>? Body { get; set; }

        [Input("cacheBustingQueryParamName")]
        public Input<string>? CacheBustingQueryParamName { get; set; }

        [Input("failIfBodyMatchesRegexps")]
        private InputList<string>? _failIfBodyMatchesRegexps;
        public InputList<string> FailIfBodyMatchesRegexps
        {
            get => _failIfBodyMatchesRegexps ?? (_failIfBodyMatchesRegexps = new InputList<string>());
            set => _failIfBodyMatchesRegexps = value;
        }

        [Input("failIfBodyNotMatchesRegexps")]
        private InputList<string>? _failIfBodyNotMatchesRegexps;
        public InputList<string> FailIfBodyNotMatchesRegexps
        {
            get => _failIfBodyNotMatchesRegexps ?? (_failIfBodyNotMatchesRegexps = new InputList<string>());
            set => _failIfBodyNotMatchesRegexps = value;
        }

        [Input("failIfHeaderMatchesRegexps")]
        private InputList<Inputs.SyntheticMonitoringCheckSettingsHttpFailIfHeaderMatchesRegexpArgs>? _failIfHeaderMatchesRegexps;
        public InputList<Inputs.SyntheticMonitoringCheckSettingsHttpFailIfHeaderMatchesRegexpArgs> FailIfHeaderMatchesRegexps
        {
            get => _failIfHeaderMatchesRegexps ?? (_failIfHeaderMatchesRegexps = new InputList<Inputs.SyntheticMonitoringCheckSettingsHttpFailIfHeaderMatchesRegexpArgs>());
            set => _failIfHeaderMatchesRegexps = value;
        }

        [Input("failIfHeaderNotMatchesRegexps")]
        private InputList<Inputs.SyntheticMonitoringCheckSettingsHttpFailIfHeaderNotMatchesRegexpArgs>? _failIfHeaderNotMatchesRegexps;
        public InputList<Inputs.SyntheticMonitoringCheckSettingsHttpFailIfHeaderNotMatchesRegexpArgs> FailIfHeaderNotMatchesRegexps
        {
            get => _failIfHeaderNotMatchesRegexps ?? (_failIfHeaderNotMatchesRegexps = new InputList<Inputs.SyntheticMonitoringCheckSettingsHttpFailIfHeaderNotMatchesRegexpArgs>());
            set => _failIfHeaderNotMatchesRegexps = value;
        }

        [Input("failIfNotSsl")]
        public Input<bool>? FailIfNotSsl { get; set; }

        [Input("failIfSsl")]
        public Input<bool>? FailIfSsl { get; set; }

        [Input("headers")]
        private InputList<string>? _headers;
        public InputList<string> Headers
        {
            get => _headers ?? (_headers = new InputList<string>());
            set => _headers = value;
        }

        [Input("ipVersion")]
        public Input<string>? IpVersion { get; set; }

        [Input("method")]
        public Input<string>? Method { get; set; }

        [Input("noFollowRedirects")]
        public Input<bool>? NoFollowRedirects { get; set; }

        [Input("proxyUrl")]
        public Input<string>? ProxyUrl { get; set; }

        [Input("tlsConfig")]
        public Input<Inputs.SyntheticMonitoringCheckSettingsHttpTlsConfigArgs>? TlsConfig { get; set; }

        [Input("validHttpVersions")]
        private InputList<string>? _validHttpVersions;
        public InputList<string> ValidHttpVersions
        {
            get => _validHttpVersions ?? (_validHttpVersions = new InputList<string>());
            set => _validHttpVersions = value;
        }

        [Input("validStatusCodes")]
        private InputList<int>? _validStatusCodes;
        public InputList<int> ValidStatusCodes
        {
            get => _validStatusCodes ?? (_validStatusCodes = new InputList<int>());
            set => _validStatusCodes = value;
        }

        public SyntheticMonitoringCheckSettingsHttpArgs()
        {
        }
        public static new SyntheticMonitoringCheckSettingsHttpArgs Empty => new SyntheticMonitoringCheckSettingsHttpArgs();
    }
}
